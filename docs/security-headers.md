...
Stellar Machina applies its baseline browser security posture from `next.config.ts` by calling `defaultSecurityHeaders()` in `src/lib/securityHeaders.ts` for every route (`/:path*`). The header map is generated at build time so the shipped Content-Security-Policy (CSP) matches the same API origin the client code will use.
...
| `connect-src` | `'self'` plus the resolved API origin | Allows browser fetch/WebSocket-style connections to the dashboard origin and the Stellar Machina API origin derived from `NEXT_PUBLIC_AGENTPAY_API_BASE`. |
